...
export async function POST(request: Request) {
  ...
  try {
    ...
    const requestPath =
      process.env.NODE_ENV === 'development' ? 'http://127.0.0.1:8000/api/python' : '/api/index'; //`${request.headers.get('host')}/api/python`; 'https://sentiment-analysis-seven.vercel.app/api/python'; /api/python
